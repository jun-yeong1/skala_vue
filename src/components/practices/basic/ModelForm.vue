...
<template>
  <div class="practice-section">
    ...
    <div>
      <h3>4) Radio (단일 선택)</h3>
      <label><input type="radio" value="치킨" v-model="food" /> 치킨</label> &nbsp;
      <label><input type="radio" value="피자" v-model="food" /> 피자</label>
      <p>
        데이터 상태: <span>{{ food }}</span>
      </p>
    </div>
    <div>
      <h3>5) Select (드롭다운 선택)</h3>
      <select v-model="selectedCk">
